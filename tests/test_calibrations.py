...
def test_measurand_quantity_attribute_unique(xml):
    m1 = xml.measurand(quantity='Humidity', calibrationInterval='5')
    m2 = xml.measurand(quantity='Temperature', calibrationInterval='5')
...
def test_measurand_quantity_attribute_not_unique(xml, quantity):
    m1 = xml.measurand(quantity=quantity, calibrationInterval='5')
    m2 = xml.measurand(quantity=quantity, calibrationInterval='1')
    xml.calibrations(f'{m1}  {m2}')
...
@pytest.mark.parametrize(
    ('attribs', 'missing'),
    [({'foo': 'bar'}, ''),
     ({'quantity': 'Humidity'}, 'calibrationInterval'),
     ({'calibrationInterval': '0'}, 'quantity'),
     ])
...
def test_measurand_invalid_attribute_name(xml):
    xml.calibrations(xml.measurand(quantity='Humidity', calibrationInterval='5', foo='bar'))
    xml.raises("measurand', attribute 'foo'")


def test_measurand_invalid_quantity_value(xml):
    xml.calibrations(xml.measurand(quantity='invalid', calibrationInterval='5'))
...
def test_measurand_invalid_interval_value(xml, interval):
    xml.calibrations(xml.measurand(quantity='Humidity', calibrationInterval=interval))
